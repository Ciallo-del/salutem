...
public class CustomerSettlePreSheetController extends DefaultBaseController {
  ...
  public InvokeResult<PageResult<QueryCustomerSettlePreSheetBo>> query(
    ...
    PageResult<CustomerSettlePreSheet> pageResult = settlePreSheetService.query(getPageIndex(vo),
        getPageSize(vo), vo);
    ...
    if (!CollectionUtil.isEmpty(datas)) {
      results = datas.stream().map(QueryCustomerSettlePreSheetBo::new).collect(Collectors.toList());
    }
